...
public class BurgerControllerTest {
  ...
  public void createBurger_twoBurgersWithDuplicateName_shouldCreateOnlyFirstBurger() {
    ...
    ResponseEntity<BurgerResponse> burgerResponse1 = burgerTestFactory.createTestBurgerAndValidate(
        equalName,
        price1);
    ...
    ResponseEntity<String> burgerResponse2 = burgerTestFactory.createTestBurgerExpectingError(
        equalName,
        price2);
  ...
  public void createBurger_and_findAllBurgers_shouldFindAllCreatedBurgers() {
    ...
    BurgerResponse burgerResponse2 = burgerTestFactory.createTestBurgerAndValidate(name2, price2)
        .getBody();
    Long newId2 = burgerResponse1.getId();

    ParameterizedTypeReference<List<BurgerResponse>> responseType = new ParameterizedTypeReference<>() {
    };

    ResponseEntity<List<BurgerResponse>> getResponse = testRestTemplate.exchange(
        "/api/burger",
        HttpMethod.GET,
        null,
        responseType);
    ...
    BurgerResponse foundBurger1 = allBurgers.stream()
        .filter(burger -> name1.equals(burger.getName())).findFirst()
        .orElseThrow(() -> new AssertionError("Burger 1 not found."));

    BurgerResponse foundBurger2 = allBurgers.stream()
        .filter(burger -> name2.equals(burger.getName())).findFirst()
    ...
    assertThat(foundBurger1.getId()).isEqualTo(newId2);
  ...
  public void updateBurger_newNameButTheSamePrice_shouldSuccess() {
    ...
    BurgerResponse burgerResponse1 = burgerTestFactory.createTestBurgerAndValidate(
        originName,
        originPrice).getBody();
    ...
    ResponseEntity<BurgerResponse> burgerResponse2 = testRestTemplate.exchange(
        "/api/burger/" + newId,
        HttpMethod.PUT,
  ...
  public void updateBurger_newPriceButTheSameName_shouldSuccess() {
    ...
    BurgerResponse burgerResponse1 = burgerTestFactory.createTestBurgerAndValidate(originName,
        originPrice).getBody();
    Long newId = burgerResponse1.getId();
  ...
  public void updateBurger_newNameButTheNameIsAlreadyExists_shouldFail() {
    ...
    ResponseEntity<String> burgerResponse = testRestTemplate.exchange(
        "/api/burger/" + newId1,
        HttpMethod.PUT,
  ...
  public void deleteBurger_ifTheBurgerIsNotExists_shouldFail() {
    ResponseEntity<Void> response = testRestTemplate.exchange(
        "/api/burger/8888", HttpMethod.DELETE,
        null, Void.class);
  ...
  public void deleteBurger_createThreeBurgersAndCheckIfTheListOfBurgersContainsOnlyOne() {
    ...
    BurgerResponse burgerResponse3 = burgerTestFactory.createTestBurgerAndValidate(name3, price3)
        .getBody();
    ...
    ResponseEntity<Void> deleteBurgerResponse3 = testRestTemplate.exchange(
        "/api/burger/" + newId3,
        HttpMethod.DELETE,
        null,
        Void.class);
